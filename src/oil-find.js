...
(function () {
  ...
  /* 명단 페이지는 GitHub Pages 쪽에 있다(블로그스팟에는 파일을 올릴 수 없다).
     주소를 여기 한 줄로 둔다 - 연휴가 끝나면 이 줄과 아래 띠만 지운다. */
  var CHUSEOK = 'https://letsdoooitdo-dot.github.io/search-oil/chuseok/';
  ...
  function renderFind(meta, reg) {
    ...
    /* 추석 연휴 임시 띠 (2026-09-25 ~ 09-27).
       블로그·쓰레드에 "100원 할인 주유소 명단은 사이트에" 로 올려 사람을
       불러오는 중이다. 들어온 사람이 첫 화면에서 한 번에 받게 해야 한다 -
       여기가 그 자리다.
       ★ 9/28 이후에는 이 블록만 지우면 된다. 다른 데는 건드리지 않았다. */
    html += '<a class="oil-chuseok" href="' + CHUSEOK + '">' +
      '<span class="oil-chuseok-tag">추석 연휴 한정 · 9월 27일까지</span>' +
      '<span class="oil-chuseok-h">100원 내린 고속도로 주유소 205곳</span>' +
      '<span class="oil-chuseok-s">명단 보기 · 엑셀로 내려받기 ⤓</span></a>';
    ...
    /* 자주 쓰는 분께만 보이는 귀띔. 기능 상자들 바로 위, 언제나 같은 자리다.
       앱 안에서 매번 위치 팝업을 보는 게 바로 이 버튼들을 누를 때라, 화면 맨
       아래에 두면 정작 겪는 순간에는 눈에 안 들어온다.
       ★ if 안에 넣지 말 것. 출발지를 고르러 온 경우에는 '내 주변' 상자가
         없어서, 조건 안에 두면 그때만 귀띔이 통째로 사라진다. */
